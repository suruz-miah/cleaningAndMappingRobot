...
\labday{Thursday, January 21, 2021}
\experiment{Laboratory work}
NS\\
...
\labday{Saturday, February 6, 2021}
\experiment{Lab work}
NS\\ 
...
\experiment{printer specifications}
\begin{itemize}
...
\labday{Monday, May 17, 2021}
\experiment{Lab work}
NS\\
Jack and I began designing the new robot in Fusion 360 using T slot extrusion as the frame of the body. We will most likely use 3D printed parts for the walls and shell of the robot. 3D printed parts might also be used to make more intricate parts that would otherwise be difficult to manufacture.

\labday{Tuesday, May 18, 2021}
\experiment{Meeting minutes}
\begin{itemize}
\item Updated Md. Miah on new design of robot using T slots.
\item Talked about moving away from the old design to use T slot extrusions instead
\begin{itemize}
\item In addion to moving away from old design we will also be moving away from the gear train design to power the front wheels and will use stronger motors to directly drive the wheels.
\end{itemize}

\begin{figure}[H]
\begin{subfigure}{.5\textwidth}
\centering
\includegraphics[width=1.5\linewidth]{figs/img/v2Body}
\subcaption{Old design}
\end{subfigure}

\begin{subfigure}{.5\textwidth}
\centering
\includegraphics[width=1.5\linewidth]{figs/img/v3Body}
\subcaption{New design}
\end{subfigure}
\caption{Body design change}
