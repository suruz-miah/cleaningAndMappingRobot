...
Today, I drew the below figure in Inkscape:
\begin{figure}[H]
\centering
...
Description:
\smallbreak\noindent
The proposed cleaning and mapping robot will have the ability to collect sensor data from multiple onboard sensors like an IR sensor, cliff sensor, gyroscope sensor, IMU, LIDAR, and ultrasonic sensor connected to a single board computer (Beaglebone Blue) for the purpose of localization and mapping. With the aid of the computer, the robot will be able to successfully map its environment like a floor of a house or building. For visualizing the fully completed map, an Android app will be developed to display the outcome of the mapping graphically. Built-in WiFi support of the Beaglebone will act as the communication medium between the robot and client. Other 
\end{addmargin}
